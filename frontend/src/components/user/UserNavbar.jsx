...
function UserNavbar() {
    ...
    // Sort the notifications array in descending order of timeStamp
    const sortedNotifications = notifications.sort((a, b) => new Date(b.timeStamp) - new Date(a.timeStamp));

    const handleNotificationClick = (notification) => {
        // Check if notification contains a property ID
        if (notification.propertyId) {
            // Redirect to property overview page
            window.location.href = `/user/propertyOverview/${notification.propertyId}`;
        }
    };
                    ...
                    <div className="absolute right-0 mt-2 w-64 bg-white shadow-xl rounded-lg p-4">
                        ...
                        <ul>
                            {sortedNotifications.map((notification, index) => (
                                <li key={index} className={`${notification.status ? 'opacity-50' : ''} mt-2`} onClick={() => handleNotificationClick(notification)}>
